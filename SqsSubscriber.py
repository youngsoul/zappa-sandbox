...
def sqs_listener(event, context):
    logger.debug("SqsSubscriber.sqs_listener")
    logger.debug(f"event: {event}")
    logger.debug(f"context: {context}")
    logger.debug(f"Message Body: {event['Records'][0]['body']}")
    if 'messageAttributes' in event['Records'][0]:
        logger.debug(f"Message Attributes: {event['Records'][0]['messageAttributes']}")
